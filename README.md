...
Простая игра Tower Defense на Windows Forms с акцентом на чистую архитектуру (Factory, Strategy, State) и наглядную визуализацию.

## Возможности
- Фазы строительства и волны (можно строить башни во время волны)
- Типы башен: Cannon (урон), Slow (замедление)
- Типы врагов: Fast, Tank
- Динамическая сложность (HP растет с волнами и количеством башен)
- Графика: антиалиасинг, трава+дорога, радиусы атаки, линии выстрелов, полоски HP

## Структура
- TowerDefense/ — UI проект (Windows Forms)
- TowerDefense.Core/ — игровая логика (Entities, Factories, States, Strategies)
- TowerDefense.Tests/ — тесты (по желанию)

## Сборка и запуск
```bash
# В каталоге Tower/
# Сборка всех проектов в решении (если есть .sln) — или по отдельности
# Пример для UI проекта:
cd "TowerDefense"
dotnet build
...
## Краткая архитектура
- Factory: `EnemyFactory`, `TowerFactory` создают типы врагов/башен
- Strategy: `DamageAttack`, `SlowAttack` — стратегия атаки башен
- State: `BuildState`, `WaveState`, `GameOverState` + `GameContext` управляет режимами

## Управление
- Кнопка Start Wave — запуск волны / рестарт
- Клик по полю — установка башни (вне зоны дороги)
- Селектор справа — выбор типа башни
...
## Замечания
- Папки `bin/`, `obj/`, `.vs/`, `packages/` и пользовательские файлы исключены в `.gitignore`
